...
float speed = 0.12; //mouseY;

void main( void )
{
    vec2 uv = v_vTexcoord;
    float niceNoise1 = FBM( vec3(80.0 * uv, speed * 60.*time))*intensity;
    float niceNoise2 = FBM( vec3(80.0 * uv, speed * 62.*time + 300.))*intensity;
    gl_FragColor = texture2D(gm_BaseTexture, uv + vec2(magn*0.2*niceNoise1,magn*0.21*niceNoise2) );
}
